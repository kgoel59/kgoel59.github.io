...
def alpha_decay(learning_rate, t, max_iter):
    ...
    """
    return learning_rate * (max_iter - t) / max_iter;
#   return learning_rate / (1+t/max_iter)

def sigma_decay(sigma, t, max_iter):
    """Decay function of the learning process.
    Parameters
    ----------
    sigma : float
    ...
    max_iter : int
        maximum number of iterations for the training.
    """
    return sigma - (sigma - 1.0) * t/max_iter;
#    return sigma / (1+t/(max_iter/2))


class MiniSom(object):
    def __init__(self, x, y, input_len, sigma=1.0, learning_rate=0.5,
                 decay_function=alpha_decay,
    ...
    def myQuantizationError(self, data):
        qError = 0;
        cnt=0
        for x in data:
            cnt += 1
            self._activate(x)
            win=self.winner(x)
            qError += linalg.norm(x - self._weights[win])
        qError = qError/cnt
        return qError
    ...
    def update(self, x, win, t, max_iteration):
        ...
        eta = alpha_decay(self._learning_rate, t, max_iteration)
        # sigma and learning rate decrease with the same rule
        #sig = self._decay_function(self._sigma, t, max_iteration)
        sig =  sigma_decay(self._sigma, t, max_iteration)
        #print("alpha:" + str(eta) + " " + str(sig))
    ...
    def train(self, data, num_iteration, random_order=False, verbose=False):
        ...
        for t, iteration in enumerate(iterations):
            #print("iter:" + str(t) + " "+ str(iteration))
            mywinner = self.winner(data[iteration])
            self.update(data[iteration], mywinner, t, num_iteration)
    ...
    def quantization_error(self, data):
        ...
        return self.myQuantizationError(data)
 #       self._check_input_len(data)
